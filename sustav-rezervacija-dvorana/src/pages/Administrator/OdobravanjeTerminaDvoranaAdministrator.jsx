...
export default function OdobravanjeTerminaDvoranaAdministrator() {
    ...
    const omoguci_stisnuto = async (id_entry) => {
    ...
    const odbaci_stisnuto = async (id_entry) => {
        const token = localStorage.getItem("token");
        const headers = { Authorization: `Bearer ${token}` };
        try {
            const response = await axios.put(`http://localhost:3000/api/odbaciEntry/${id_entry}`, {}, {headers});
            window.alert("Rezervacija dvorana je odbačena.")
        } catch (error) {
            console.log("Greška prilikom ažuriranja rezervacije:", error);
            window.alert("Došlo je do greške prilikom ažuriranja rezervacije.");
        }
    ...
    const dohvatiZahtjeve = async (id_korisnika, headers) => {
        try {
            const response = await axios.get(`http://localhost:3000/api/entryOdobravanje`, {headers});
            ...
            console.log("DATA:", modifiedData.data);
                ...
                <tbody>
                    {filteredZahtjevi.map((zahtjev, index) => (
                        <tr key={index}>
                            ...
                            <td>
                            <button className="gumb_vidi" onClick={()  => omoguci_stisnuto(zahtjev.id_entry)}>Odobri</button>
                            <button className="gumb_obriši" onClick={() => odbaci_stisnuto(zahtjev.id_entry)}>Odbaci</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
